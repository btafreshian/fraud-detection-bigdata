"""Score transactions using the saved sklearn pipeline."""
...
def score_transactions(
    ...
    *,
    ...
    """Score transactions and return fraud probabilities in the same order."""

    rows_list = list(rows)
    if not rows_list:
        return []

    active_model = model or load_model(model_path)
    df = pd.DataFrame(rows_list)
    if df.empty:
        return []

    probabilities = active_model.predict_proba(df[FEATURE_COLS])[:, 1]
...
def main() -> None:
    ...
    probs = score_transactions(rows, model=model)
