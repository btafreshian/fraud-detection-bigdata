...
@contextmanager
def get_client(host: Optional[str] = None, port: Optional[int] = None):
    """
    Context manager that yields a connected Ignite client.

    Host and port default to ``src.config`` constants but can be overridden for
    ad-hoc scripts or different environments.
    """

    host = host or config.IGNITE_HOST
    port = port or config.IGNITE_PORT

    client = Client(timeout=config.DEFAULT_SOCKET_TIMEOUT_MS)
    try:
        client.connect(host, port)
        yield client
    except Exception as exc:  # noqa: BLE001
        raise RuntimeError(f"Unable to connect to Apache Ignite at {host}:{port}") from exc
    finally:
...
def insert_transaction(tx: Dict[str, Any], *, host: Optional[str] = None, port: Optional[int] = None) -> str:
    ...
    with get_client(host=host, port=port) as client:
        client.sql(statement, query_args=[row[col] for col in TRANSACTION_COLUMNS])
    return row["tx_id"]


def insert_transactions(
    rows: Iterable[Dict[str, Any]], *, host: Optional[str] = None, port: Optional[int] = None
) -> List[str]:
...
def fetch_transactions(
    limit: int = config.DEFAULT_FETCH_LIMIT, *, host: Optional[str] = None, port: Optional[int] = None
) -> List[Dict[str, Any]]:
